...
$fileUrl = 'live_import_customer.csv';
...
if (($handle = @fopen($fileUrl, 'r')) !== FALSE) {
	...
	foreach ($data as $csvdata) {
		...
		$customer->setPassword("larsen@123");
		...
		if ($Billing_Address1 || $Billing_City || $Billing_State || $Billing_Zip || $Billing_Country) 
		{
		...
		if ($Shipping_Address1 || $Shipping_City || $Shipping_State || $Shipping_Zip || $Shipping_Country) 
		{
			echo "shipping" . "\n";
			$addresss = $obj->get('\Magento\Customer\Model\AddressFactory');
		...
		$writer = new \Zend\Log\Writer\Stream(BP . '/var/log/customer_with_address_19_7_2020.log');
