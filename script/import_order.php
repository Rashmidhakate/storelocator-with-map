		...
		foreach ($values as $key => $value) {
			if ($headers[0][$key] == "OrderNumber") {
				...
				if ($headers[0][$key] == 'OrderedProductSKU') {
					...
					$item['sku'] = $value;
				} else if ($headers[0][$key] == 'OrderedProductName') {
					if ($value == 'NULL' || $value == '') {
						$value = 'Value Missing';
					}
					$item['name'] = $value;
					...
					$item['size'] = $value;
				} else if ($headers[0][$key] == 'Product_Width_Range') {
					$item['width'] = $value;
...
foreach ($orders as $csvdata) {
	...
	$orderCount++;
	echo $orderCount . "\n";
	$OrderNumber = 'U-'.$csvdata["OrderNumber"];
	...
	/* -- date format-- */
	// $ShippedOnDate = explode(' ', (string) $ShippedOn);
	// $ShippedOn = date_format(date_create_from_format('d/m/y', $ShippedOnDate[0]), 'Y-m-d') . ' ' . $ShippedOnDate[1];
	// $shippon_date_timestamp = strtotime($ShippedOn);
	// $shippon = date('Y-m-d H:i:s', $shippon_date_timestamp);
	//echo $shippon . "\n";

	// $OrderDate = explode(' ', (string) $OrderDate);
	// $OrderDate = date_format(date_create_from_format('d/m/y', $OrderDate[0]), 'Y-m-d') . ' ' . $OrderDate[1];
	// $orderon_date_timestamp = strtotime($OrderDate);
	// $orderon = date('Y-m-d H:i:s', $orderon_date_timestamp);
	//echo $orderon . "\n";
	
	$authoriZedDate = explode(' ', (string) $AuthorizedOn);
	$newDateString = date_format(date_create_from_format('d/m/y', $authoriZedDate[0]), 'Y-m-d') . ' ' . $authoriZedDate[1];
	$AuthorizedOn_date_timestamp = strtotime($newDateString);
	$authorizeon = date('Y-m-d H:i:s', $AuthorizedOn_date_timestamp);
	echo $authorizeon . "\n";

	// $CapturedOn = explode(' ', (string) $CapturedOn);
	// $CapturedOn = date_format(date_create_from_format('d/m/y', $CapturedOn[0]), 'Y-m-d') . ' ' . $CapturedOn[1];
	// $CapturedOn_date_timestamp = strtotime($CapturedOn);
	// $captureon = date('Y-m-d H:i:s', $CapturedOn_date_timestamp);
	//echo $captureon . "\n";

	// $RefundedOn = explode(' ', (string) $RefundedOn);
	// $RefundedOn = date_format(date_create_from_format('d/m/y', $RefundedOn[0]), 'Y-m-d') . ' ' . $RefundedOn[1];
	...
	$option = array();
	$OrderNumber = 'U-'.$csvdata["OrderNumber"];
	$order = $obj->create('Magento\Sales\Model\Order')->loadByAttribute('increment_id', $OrderNumber);
	
	// echo $order->getWebsiteId(); die;
	if(!$order->getId() && $order->getWebsiteId() != 3) {
		// $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/order_not_vailable_11_10_2019.log');
		...
		// $logger->info($OrderNumber);
			...
			$shippingRegionId = "SELECT default_name FROM `directory_country_region` WHERE `country_id` = '" . trim($shippingCountryId) . "' AND `code` = '" . trim($ShippingState) . "' ";
			...
			$billingRegionId = "SELECT default_name FROM `directory_country_region` WHERE `country_id` = '" . trim($billingCountryId) . "' AND `code` = '" . trim($BillingState) . "' ";
			...
			$order = $orderFactory->create()->setWebsiteId(3);
			$order->setStoreId(3);
			$order
			...
			//exit;
			$writer = new \Zend\Log\Writer\Stream(BP . '/var/log/14-10-2019-create_new_order.log');
			$logger = new \Zend\Log\Logger();
			$logger->addWriter($writer);
